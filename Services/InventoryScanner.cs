...
namespace CriticalCommonLib.Services
{
    ...
    public class InventoryScanner : IInventoryScanner
    {
        ...
        [Signature("48 89 6C 24 ?? 48 89 74 24 ?? 57 48 81 EC ?? ?? ?? ?? 45 0F B7 48", DetourName = nameof(ContainerInfoDetour), UseFlags = SignatureUseFlags.Hook)]
        private Hook<ContainerInfoNetworkData>? _containerInfoNetworkHook = null;

        [Signature(
            "48 89 5C 24 ?? 48 89 74 24 ?? 57 48 81 EC ?? ?? ?? ?? 41 0F B7 40 ?? 49 8B F8 48 8B F1 3D ?? ?? ?? ?? 74 ?? 3D ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? 48 8B 59 ?? 48 85 DB 74 ?? 41 8B 40 ?? 0F 1F 40 00 39 43 ?? 74 ?? 48 8B 5B ?? 48 85 DB 75 ?? 41 8B 50 ?? E8 ?? ?? ?? ?? 48 8B D8 48 85 C0 0F 84 ?? ?? ?? ?? 0F B7 4F ?? 4C 8D 44 24 ?? 0F 10 47 ?? 89 4C 24",
